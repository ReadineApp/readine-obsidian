...
// PURPOSE: Declares IUnpackerPlugin with a single echo(options) method that dispatches base64-encoded payloads to native and resolves with {reqId, unpacked, error}. Default-exported via registerPlugin<IUnpackerPlugin>('Unpacker').
...
// START_MODULE_MAP
// IUnpackerPlugin - plugin contract: echo({reqId, action, articleDataBase64, dictDataBase64, imageDataBase64}) -> {reqId, unpacked, error}
